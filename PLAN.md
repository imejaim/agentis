...
> 작성: 코부장 / 2026-05-13 ~ 14 야간 + 후속. 8차 피드백 반영(**키트 우선·씨드는 폴백** 규약 + **씨드 자체의 공유와 진화**(절 단위 팩 머지) → 두 층 메타포의 자기유사성 확보).
...
- **Phase 1.3 (v1.3 — 키트 우선 + 씨드 자체의 진화)** ✅ — 두 가지 큰 변화: ① 결정론 두 번째 기둥 **키트 우선·씨드는 폴백** 규약을 씨드 §0-1·§1-4·§3-6·§6·§7 에 박고, 키트 핵심 .py 6개에 `# agentis-kit: v1.3` 시그니처 + `.kit-version` 마커 추가. ② **씨드 자체의 공유와 진화** — 씨드 각 절에 `<!-- @section: N -->` 앵커 박고, 새 워크플로우 `씨드-업그레이드.py`(--check/--apply/--export-branch) 추가. 사용자의 씨드 개인 진화를 보존하면서 표준 씨드 개선을 절 단위 팩으로 받음. 메타포의 자기유사성: 에이전트 층(브랜치/팩/메인) ↔ 씨드 층(동일 메타포). → **다음: 실제 브랜치 송수신·리브랜드 한 사이클 + 메인 첫 운영 시나리오.**
...
10. ✅ **v1.3 키트 우선 + 씨드 자체의 진화** — (a) 씨드 §0-1 자가점검·§1-4 키트 우선 부팅·§6 새 원칙 + 키트 6개 .py 시그니처 + `.kit-version` 마커. (b) 씨드 절(§) 앵커 + 새 워크플로우 `씨드-업그레이드.py` — `--check`/`--apply --plan`/`--export-branch` 3모드 검증 통과 (self-compare UNCHANGED, 디프 시나리오 정확 분류, apply 머지 결과 검증). 예제 폴더에도 동기화.
11. ⏭️ **사용자 실전** — 동료 한 명과 브랜치 송수신·리브랜드 한 사이클 → 피드백.
12. ⏭️ (피드백 후) Phase 2: 사내 깃 메인 첫 운영 절차, 컨셉/도입 문서, lint, 스킬 라이브러리 확충, 씨드 메인 큐레이션 첫 사이클.
...
*— 코부장 / v0.9. 다음 마디는 사내에서 한 사이클 굴려본 뒤.*
